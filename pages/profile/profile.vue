    ...
    <div class="max-w-4xl mx-auto px-4">
      ...
      <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
        <div class="flex items-center gap-6">
          <div class="relative">
            <img 
              :src="profile.imageUrl || '/favicon.ico'" 
              alt="Profile picture"
            ...
            <label class="absolute bottom-0 right-0 p-2 bg-white rounded-full shadow-lg cursor-pointer hover:scale-105 transition-transform">
              <input type="file" class="hidden" @change="handleImageUpload" accept="image/png, image/jpeg, image/jpg" />
...
const handleImageUpload = async (event) => {
  ...
  if (file) {
    ...
    reader.onload = async (e) => {
      ...
      try {
        const response = await $axios.post('/api/user/update-details', formData, {
          headers: {
